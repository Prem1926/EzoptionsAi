...
import { Search, RefreshCw, User, Eye, ArrowUp, ArrowDown, Newspaper } from 'lucide-react';
...
const EvergreenStocks = () => {
  ...
  const fetchStockTrades = async(isManualRefresh = false) => {
    if (isManualRefresh) {
      setLoading(true);
    }
    setError(null);
    try {
      const response = await axios.get('https://ezoptionsai.com/api3/evergreen-stocks');
  ...
  useEffect(() => {
    // Initial data fetch
    fetchStockTrades(true);
    fetchNews(true);

    // Set up interval to refresh data every 5 seconds
    const stocksIntervalId = setInterval(() => {
      fetchStockTrades();
    }, 5000);
    ...
    return () => {
      clearInterval(stocksIntervalId);
    ...
    <div className="min-h-screen bg-gray-100 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          {/* Main Content - Evergreen Stocks Dashboard */}
